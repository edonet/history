...
export default class History {
    ...
    /* 初始化对象 */
    constructor({ getUserConfirmation, ...options }) {

        // 创建历史对象
        this.history = createHashHistory({
            ...options,
            getUserConfirmation: (message, callback) => {
                if (typeof this.userConfirmation === 'function') {
                    let res = this.userConfirmation(message, callback);

                    if ('then' in res) {
                        res.then(callback, () => callback(false));
                    }
                } else {
        ...
        // 定义属性
        this.method = 'REPLACE';
        this.userConfirmation = getUserConfirmation;
        this.listeners = [];
        this.cache = {};
